...
// Fetch all movies by page
export const fetchAllMoviesByPage = createAsyncThunk(
    'movies/fetchAllMoviesByPage',
    async (page, { rejectWithValue }) => {
        try {
            ...
            const response = await api.get(`/movie/popular?page=${pageNumber}`);
            ...
            return rejectWithValue('Failed to fetch movies by page');
        }
    }
);
...
const moviesSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchAllMoviesByPage.pending, (state) => {
                state.loading = true;
            })
            .addCase(fetchAllMoviesByPage.fulfilled, (state, action) => {
                state.loading = false;
                state.movies = action.payload.movies;
                state.totalPages = action.payload.totalPages;
                state.currentPage = action.payload.currentPage;
            })
            .addCase(fetchAllMoviesByPage.rejected, (state, action) => {
                state.loading = false;
                state.error = action.payload;
            });
    },
